...
- Storybook contributor rules: `docs/storybook.md` (theme layers, Controls, Actions, play tests, a11y, visual checklist, consumer `source.code` snippets, CodePanel copy). Sidebar: **Hooks** / **Components**, Core Concepts (including imperative mutations + normalizeErrors), Validation / Fields / Examples, plus Migration / API / Limitations / Theme (`storySort` in `.storybook/preview.ts`). `useForm` canvas is Login; `useFieldState` uses FieldStateForm. Snippet highlighting: `--docs-syntax-*` + `syntax.css`.
